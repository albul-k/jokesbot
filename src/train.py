...
def main():
    ...
    cur = sql.connect('common/jokes.db').execute('SELECT theme,text FROM joke')
    rows = cur.fetchall()
    cur.close()
    ...
    with open(os.path.join('common', 'label_encoder.pkl'), 'wb') as file:
        pickle.dump(le, file)
    ...
    with open(os.path.join('common', 'model_svc.pkl'), 'wb') as file:
        pickle.dump(svc, file)

    modelFT = FastText(sentences=MyIter(df), size=30, min_count=1, window=5, workers=8)
    modelFT.save(os.path.join('common', 'model_FT.ft'))
    ...
    ft_index.build(10)
    ft_index.save(os.path.join('common', 'ft_index.ann'))

    with open(os.path.join('common', 'index_map.pkl'), 'wb') as file:
        pickle.dump(index_map, file)

    with open(os.path.join('common', 'tfidf.pkl'), 'wb') as file:
